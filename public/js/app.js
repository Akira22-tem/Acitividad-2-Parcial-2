document.addEventListener('DOMContentLoaded', () => {
  ...
  if (formularioCliente && listaClientes) {
    ...
    formularioCliente.addEventListener('submit', (evento) => {
      ...
      if (idClienteEdicion) {
        ...
        clientes = clientes.map((cliente) =>
          cliente.id === idClienteEdicion
            ? { ...cliente, nombre, cedula, direccion }
            : cliente
        );
        idClienteEdicion = null; // Resetear el ID de edición
        formularioCliente.querySelector('button[type="submit"]').textContent =
          'Registrar Cliente';
      } else {
    ...
    listaClientes.addEventListener('click', (evento) => {
      ...
      if (
        evento.target.matches('.boton-eliminar-cliente') &&
  ...
  if (formularioProducto && listaProductos) {
    ...
    const mostrarProductos = () => {
      ...
      productos.forEach(
        ...
        <tr>
          ...
          <td>
            <button class="btn btn-warning btn-sm boton-editar-producto" data-id="${
              producto.id
            }">Editar</button>
            <button class="btn btn-danger btn-sm boton-eliminar-producto" data-id="${
              producto.id
            }">Eliminar</button>
          </td>
        </tr>`)
    ...
    formularioProducto.addEventListener('submit', (evento) => {
      ...
      if (idProductoEdicion) {
        ...
        productos = productos.map((producto) =>
          producto.id === idProductoEdicion
            ? { ...producto, nombre, codigo, precio }
            : producto
        );
        idProductoEdicion = null; // Resetear el ID de edición
        formularioProducto.querySelector('button[type="submit"]').textContent =
          'Registrar Producto';
      } else {
    ...
    listaProductos.addEventListener('click', (evento) => {
      ...
      if (
        evento.target.matches('.boton-eliminar-producto') &&
  ...
  const listaProductosFactura = document.getElementById(
    'productos-factura-lista'
  );
  ...
  if (formularioFactura) {
        ...
        .map(
          (producto) =>
            `<option value="${producto.id}">${producto.nombre} - ${
              producto.codigo
            } ($${producto.precio.toFixed(2)})</option>`
        )
        .join('');
    /*--------------------------------------------------------------------------------------------------------------------*/
    ...
    if (idFacturaEdicion) {
      ...
      const facturaEditar = facturas.find(
        (factura) => factura.id === +idFacturaEdicion
      );
      if (facturaEditar) {
        selectorCliente.value = facturaEditar.clienteId;
        articulos = [...facturaEditar.items];
        formularioFactura.querySelector('button[type="submit"]').textContent =
          'Actualizar Factura';
      }
    }
    /*--------------------------------------------------------------------------------------------------------------------*/
    const mostrarArticulos = () => {
      if (!articulos.length) {
        listaProductosFactura.innerHTML =
          '<p>No hay productos agregados a la factura.</p>';
      ...
      elementoResumenFactura.textContent = `Total General: $${totalGeneral.toFixed(
        2
      )}`;
      listaProductosFactura
        .querySelectorAll('.boton-eliminar-articulo')
        .forEach((boton) =>
    ...
    botonAgregarProducto.addEventListener('click', () => {
      ...
      if (!idProducto || cantidad < 1) {
        alert(
          'Por favor seleccione un producto y especifique una cantidad válida.'
        );
        return;
      }

      const productoSeleccionado = datosProductos.find(
    ...
    formularioFactura.addEventListener('submit', (evento) => {
      ...
      if (!selectorCliente.value || !articulos.length) {
        alert(
          'Por favor seleccione un cliente y agregue al menos un producto.'
        );
        return;
      }
  ...
  if (elementoHistorial) {
    ...
    const mostrarHistorial = () => {
      ...
      facturas.forEach((factura) => {
        const cliente =
          datosClientes.find((c) => c.id === factura.clienteId) || {};
        ...
        contenidoHtml += `
          <div class="elemento-historial">
            <p><strong>Factura N° ${factura.id}</strong> - ${
          cliente.nombre || 'Cliente no encontrado'
        } - ${fechaFormateada} - Total: $${totalFactura.toFixed(2)}</p>
            <button class="alternar-detalles" data-id="${
              factura.id
            }">Ver Detalles</button>
            <button class="boton-editar-factura" data-id="${
              factura.id
            }">✏️ Editar</button>
            <button class="boton-eliminar-factura" data-id="${
              factura.id
            }">🗑️ Eliminar</button>
            <div class="detalles-factura" id="detalles-${
              factura.id
                ...
                <tbody>
                  ${factura.items
                    .map(
                      ...
                      <td>${articulo.cantidad}</td>
                      <td>$${articulo.precio.toFixed(2)}</td>
                      <td>$${(articulo.precio * articulo.cantidad).toFixed(
                        2
                      )}</td>
                    </tr>`
                    )
      ...
      elementoHistorial
        .querySelectorAll('.alternar-detalles')
        .forEach((boton) =>
          boton.addEventListener('click', () => {
            const divDetalles = document.getElementById(
              'detalles-' + boton.dataset.id
            );
      ...
      elementoHistorial
        .querySelectorAll('.boton-eliminar-factura')
        .forEach((boton) =>
          boton.addEventListener('click', () => {
            if (
              confirm(
                '¿Está seguro de eliminar esta factura? Esta acción no se puede deshacer.'
              )
            ) {
              facturas = facturas.filter(
                (factura) => factura.id !== +boton.dataset.id
              );
      ...
      elementoHistorial
        .querySelectorAll('.boton-editar-factura')
        .forEach((boton) =>
          boton.addEventListener('click', () => {
...
// conduce el evento de instalación de la PWA
let deferredPrompt;
window.addEventListener('beforeinstallprompt', (e) => {
  e.preventDefault(); // Previene que el navegador muestre el prompt automático
  deferredPrompt = e; // Guarda el evento para que pueda ser activado más tarde
